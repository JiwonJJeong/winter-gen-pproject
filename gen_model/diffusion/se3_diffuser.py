# Adapted from https://github.com/jasonkyuyim/se3_diffusion/tree/master/ProteinMPNN
...
# Utility function for quat_to_rotvec
def quat_to_rotvec(quat):
    """Convert quaternion to rotation vector."""
    # Normalize quaternion
    quat = quat / torch.norm(quat, dim=-1, keepdim=True)
    # Extract angle
    angle = 2 * torch.acos(torch.clamp(quat[..., 0], -1, 1))
    # Extract axis
    sin_half_angle = torch.sqrt(1 - quat[..., 0]**2)
    axis = quat[..., 1:] / (sin_half_angle[..., None] + 1e-8)
    # Rotation vector
