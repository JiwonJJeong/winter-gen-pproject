...
4. ~~Wire `SpatioTemporalAttention` into `IpaScore` behind `star.enabled` flag; update signature to accept `frame_idx` and `cond`; add `star` sub-config to `default_model_conf`; extend LoRA `target_modules`~~ ✅ Done
